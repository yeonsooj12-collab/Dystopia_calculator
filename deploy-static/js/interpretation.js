...
const PAIR_LABELS = {
  "pressure-target": "사회문제와 대상",
  "pressure-technology": "사회문제와 기술",
  "pressure-transformation": "사회문제와 제도 변화",
  "pressure-ideology": "사회문제와 정당화 명분",
  "target-technology": "대상과 기술",
  "target-transformation": "대상과 제도 변화",
  "technology-transformation": "기술과 권리·제도 변화",
  "transformation-ideology": "제도 변화와 정당화 명분",
};
...
export function getSelectionText(selection, categoryKey) {
  const option = selection?.core?.[categoryKey] || selection?.detail?.[categoryKey] || selection?.amplifier?.[categoryKey];
...
export function getMissingCoreCategories(selection = {}) {
  return CORE_KEYS.filter((key) => !selection.core?.[key]).map((key) => ({ key, label: CATEGORY_LABELS[key] }));
}

function completedCoreCount(selection = {}) {
  return CORE_KEYS.filter((key) => selection.core?.[key]).length;
}
...
export function getInterpretationHeadline(evaluation = {}, selection = {}) {
  ...
  if (count === 0) return "핵심 구조를 작성하면 연결 진단을 시작할 수 있습니다.";
  if (count < 3) return "먼저 핵심 구조를 조금 더 채워보세요.";
  if (evaluation.confidence === "low") return "아직 연결을 판단할 정보가 충분하지 않습니다.";
  if (evaluation.compatibility >= 65) return "핵심 구조가 비교적 선명하게 연결됩니다.";
...
export function diagnosePressureTarget(input = {}) {
  ...
  const pressure = selection.core?.pressure;
  const target = selection.core?.target;
  const pair = findPair(evaluation, "pressure", "target");
  if (!pressure || !target) {
    return makeDiagnosis("missing", "사회문제와 대상", "사회문제와 대상 중 비어 있는 항목이 있습니다.", {
      suggestion: "먼저 어떤 문제가 누구에게 직접 작동하는지 적어보세요.",
      questions: ["이 사회문제가 어떤 집단에게 가장 먼저 영향을 주나요?"],
      relatedCategories: ["pressure", "target"],
    });
  }
  if (!pair || pair.confidence === "low") {
    return makeDiagnosis("uncertain", "사회문제와 대상", "사회문제와 대상의 연결을 판단할 정보가 아직 충분하지 않습니다.", {
      suggestion: "문제가 이 대상에게 직접 영향을 주는 이유를 한 문장 더 붙여보세요.",
      questions: ["왜 이 대상이어야 하나요?", "이 대상은 어떤 기준으로 분류되나요?"],
      relatedCategories: ["pressure", "target"],
    });
  }
  if (pair.score >= 65) {
    return makeDiagnosis("strong", "사회문제와 대상", "사회문제와 대상이 비교적 직접적으로 연결됩니다.", {
      reason: "두 항목 사이에 비교 가능한 연결 근거가 있습니다.",
      questions: ["이 대상이 제도 안에서 어떤 기준으로 분류되는지 더 구체화할 수 있나요?"],
      relatedCategories: ["pressure", "target"],
    });
  }
  if (pair.score >= 35) {
    return makeDiagnosis("workable", "사회문제와 대상", "기본 연결은 보이지만 대상이 직접 영향을 받는 이유를 더 설명하면 선명해집니다.", {
      suggestion: "문제가 이 집단의 생활, 권리, 비용 중 무엇을 건드리는지 적어보세요.",
      questions: ["이 사회문제가 이 대상에게 가장 직접적으로 주는 영향은 무엇인가요?"],
      relatedCategories: ["pressure", "target"],
    });
  }
  return makeDiagnosis("needs-detail", "사회문제와 대상", "사회문제와 대상 사이의 직접적인 연결이 아직 약하게 보입니다.", {
    suggestion: "대상을 바꾸기보다, 왜 이 대상이 제도 안에서 먼저 분류되는지 설명해보세요.",
    questions: ["이 대상은 제도에서 어떤 기준으로 분류되나요?"],
    relatedCategories: ["pressure", "target"],
  });
}
...
export function diagnoseTechnology(input = {}) {
  const selection = unpackSelection(input);
  const evaluation = input.evaluation || {};
  const technology = selection.core?.technology;
  const transformation = selection.core?.transformation;
  const text = technology?.rawText || technology?.label || "";
  if (!technology) return makeDiagnosis("missing", "기술 역할", "기술 항목이 아직 비어 있습니다.", { questions: ["이 제도가 현실에서 불가능했던 무엇을 가능하게 하나요?"], relatedCategories: ["technology"] });
  ...
  if (vagueCustom) {
    return makeDiagnosis("needs-detail", "기술 역할", "기술의 이름은 있지만 무엇을 가능하게 하는지는 아직 구체적이지 않습니다.", {
      suggestion: "기술이 판단, 예측, 감시, 배분, 자동화 중 무엇을 하는지 적어보세요.",
      questions: ["이 기술은 제도에서 정확히 무엇을 가능하게 하나요?"],
      relatedCategories: ["technology", "transformation"],
    });
  }
  if (pair?.confidence === "low") return makeDiagnosis("uncertain", "기술 역할", "기술과 제도 변화의 연결 근거가 아직 부족합니다.", { questions: ["이 기술이 없으면 왜 이 제도가 작동하지 않나요?"], relatedCategories: ["technology", "transformation"] });
  if (hasAnyTag(technology, ["functions", "enables"]) && transformation && pair?.score >= 50) {
    return makeDiagnosis(pair.score >= 65 ? "strong" : "workable", "기술 역할", "기술이 제도 변화를 가능하게 하는 방향성이 보입니다.", {
      reason: "기술의 기능과 제도 변화가 비교 가능한 방식으로 연결됩니다.",
      questions: ["기술이 사람을 측정하나요, 분류하나요, 배분하나요?"],
      relatedCategories: ["technology", "transformation"],
    });
  }
  return makeDiagnosis("workable", "기술 역할", "기술 항목은 있지만 제도 안에서의 역할을 더 구체화하면 좋습니다.", { questions: ["이 기술은 판단, 예측, 감시, 배분 중 무엇을 하나요?"], relatedCategories: ["technology"] });
}
...
export function diagnoseTransformation(input = {}) {
  ...
  if (!transformation) {
    return makeDiagnosis("missing", "권리·제도 변화", "어떤 권리나 제도가 바뀌는지 정하면 중심 모순이 더 선명해집니다.", {
  ...
  if (hasVerb && hasRight) return makeDiagnosis("strong", "권리·제도 변화", "무엇이 어떤 제도적 조건으로 바뀌는지 비교적 구체적입니다.", { questions: ["그 권리를 잃거나 얻는 기준은 무엇인가요?"], relatedCategories: ["transformation"] });
  if (hasVerb || hasRight) return makeDiagnosis("needs-detail", "권리·제도 변화", "변화의 방향은 보이지만 권리 대상이나 변환 방식 중 하나가 더 필요합니다.", { suggestion: "무엇을 점수, 허가, 구독, 상품, 자격, 배분 대상으로 바꾸는지 적어보세요.", questions: ["무엇이 배분되거나 제한되나요?"], relatedCategories: ["transformation"] });
  return makeDiagnosis("needs-detail", "권리·제도 변화", "문장은 있지만 어떤 권리나 제도가 어떻게 바뀌는지는 아직 추상적입니다.", { suggestion: "권리 이름과 변환 동사를 함께 써보세요.", questions: ["기존에 당연했던 무엇이 조건부 권리가 되나요?"], relatedCategories: ["transformation"] });
}

export function diagnoseIdeology(input = {}) {
  const selection = unpackSelection(input);
  const evaluation = input.evaluation || {};
  const ideology = selection.core?.ideology;
  const transformation = selection.core?.transformation;
  const text = ideology?.rawText || ideology?.label || "";
  if (!ideology) return makeDiagnosis("missing", "정당화 이념", "사회가 이 제도를 어떤 명분으로 받아들이는지 아직 비어 있습니다.", { questions: ["사람들은 왜 이 제도를 좋은 정책이라고 믿나요?"], relatedCategories: ["ideology"] });
  if (ideology.source === "custom" && (text.length < 8 || VAGUE_IDEOLOGY.some((term) => text.includes(term)))) {
    return makeDiagnosis("needs-detail", "정당화 이념", "명분이 아직 일반적이라 제도의 자기 정당화가 약하게 보입니다.", { suggestion: "효율, 공정, 안전, 자유, 돌봄 중 무엇을 앞세우는지 적어보세요.", questions: ["반대하는 사람을 비합리적으로 보이게 만드는 명분은 무엇인가요?"], relatedCategories: ["ideology"] });
  }
  ...
  if ((textIncludesAny(text, IDEOLOGY_TERMS) || hasAnyTag(ideology, ["tones", "rights", "functions"])) && transformation) {
    return makeDiagnosis(pair?.score >= 65 ? "strong" : "workable", "정당화 이념", "제도 변화가 내세우는 명분이 보입니다.", { questions: ["이 명분이 실제 피해를 어떻게 가리나요?"], relatedCategories: ["ideology", "transformation"] });
  }
  return makeDiagnosis("workable", "정당화 이념", "명분은 있지만 제도 변화와의 충돌을 더 설명하면 좋습니다.", { questions: ["사회는 왜 이 제도를 좋은 정책이라고 믿나요?"], relatedCategories: ["ideology"] });
}
...
export function diagnoseCoreThesis(input = {}) {
  ...
  const pair = findPair(evaluation, "transformation", "ideology");
  if (pair?.confidence === "low") return makeDiagnosis("uncertain", "중심 모순", "명분과 제도 변화의 연결은 아직 판단할 정보가 부족합니다.", { questions: ["이 명분은 어떤 권리 제한을 가리나요?"], relatedCategories: ["ideology", "transformation"] });
  return makeDiagnosis(pair?.score >= 55 ? "strong" : "workable", "중심 모순", `${getSelectionText(selection, "ideology")}이라는 명분과 ${getSelectionText(selection, "transformation")}이 연결되며, 명분과 실제 결과의 충돌을 만들 수 있습니다.`, {
    reason: "정당화 언어와 권리·제도 변화가 함께 제시되어 있습니다.",
...
export function buildPriorityInsight(input = {}) {
  ...
  if (diagnoses.transformation?.status === "needs-detail") return { title: "먼저 이 부분을 생각해보세요", message: "권리·제도 변화가 조금 더 구체적이면 중심 모순이 선명해집니다.", suggestion: diagnoses.transformation.suggestion, questions: diagnoses.transformation.questions };
  if (diagnoses.technology?.status === "needs-detail") return { title: "먼저 이 부분을 생각해보세요", message: "기술이 제도에서 맡는 역할을 더 구체화할 수 있습니다.", suggestion: diagnoses.technology.suggestion, questions: diagnoses.technology.questions };
  if (diagnoses.pressureTarget?.status !== "strong") return { title: "먼저 이 부분을 생각해보세요", message: "사회문제와 대상의 연결을 더 선명하게 만들 수 있습니다.", suggestion: diagnoses.pressureTarget.suggestion, questions: diagnoses.pressureTarget.questions };
  if (diagnoses.ideology?.status === "needs-detail") return { title: "먼저 이 부분을 생각해보세요", message: "정당화 명분을 더 구체화하면 미친 소리의 방향이 선명해집니다.", suggestion: diagnoses.ideology.suggestion, questions: diagnoses.ideology.questions };
  if (evaluation.scatterRisk >= 45 && evaluation.amplifierEvaluation?.selectedCount >= 3) return { title: "먼저 이 부분을 생각해보세요", message: "미친 소리 증폭 요소가 많아 중심 모순이 분산될 수 있습니다.", suggestion: "가장 강한 증폭 요소 1~2개만 남겨도 충분한지 확인해보세요.", questions: ["이 설정에서 가장 불편한 지점은 무엇인가요?"] };
...
export function getPromptQuestions({ diagnoses = {}, priorityInsight = {}, evaluation = {} } = {}) {
  ...
  if (evaluation.scatterRisk >= 45) add(["미친 소리 증폭 요소 중 하나만 남긴다면 무엇이 가장 핵심인가요?"]);
  return questions;
}
